...
class AbstractExporterTest extends MockeryTestCase
{
    ...
    {
        $this->fs->expects('createTempFile')->with('bungle-export-error', 'blah')->andReturn('errorFile');
        $this->exporter->expects('buildParamParser')
            ->andReturn(new ArrayIterator([$p1 = Mockery::mock(ParamValueParserInterface::class)]));
        $p1->expects('__invoke')->andReturn('blah');
        self::assertEquals(new ExportResult('errorFile', 'error.txt'), $this->exporter->export($this->context));
    }
    ...
    public function testExportExportError(): void
    {
        ...
        $this->fs->expects('createTempFile')->with('bungle-export-error', 'export error')->andReturn('errorFile');
        $this->exporter->expects('buildParamParser')->andReturn(new ArrayIterator([]));
        $this->exporter->expects('doBuild')->with('exportFile', [])->andThrow(new RuntimeException('export error'));
        self::assertEquals(new ExportResult('errorFile', 'error.txt'), $this->exporter->export($this->context));
    }
    ...
    public function testExportSucceed(): void
    {
        $this->fs->expects('createTempFile')->with('bungle-export')->andReturn('exportFile');
        $this->exporter->expects('buildParamParser')
            ->andReturn(new ArrayIterator([fn (ExportContext $ctx) => $ctx->set('a', 1)]));
        $this->exporter->expects('buildFilename')->with(['a' => 1])->andReturn('foo.xlsx');
        $this->exporter->expects('doBuild')->with('exportFile', ['a' => 1]);

        self::assertEquals(new ExportResult('exportFile', 'foo.xlsx'), $this->exporter->export($this->context));
    }
}
