...
class QueryTest extends MockeryTestCase
{
     ...
     * @depends testQuery
     */
    public function testCanNotQueryTwice(Query $q): void
    {
        $this->expectException(LogicException::class);
        $this->expectExceptionMessage('Can only query once, recreate Query object if needed');

        $params = new QueryParams(0, []);
        iterator_to_array($q->query($params));
    }

    public function testPagedQuery(): Query
    {
    ...
    /**
     * @depends testPagedQuery
     */
    public function testCanNotPagedQueryTwice(Query $q): void
    {
        $this->expectExceptionMessage('Can only query once, recreate Query object if needed');
        $this->expectException(LogicException::class);
